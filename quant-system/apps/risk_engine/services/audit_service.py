...
class AuditService:
    ...
    def audit(
        ...
        if global_risk["daily_drawdown_ratio"] >= 0.08:
            rejection_reasons.append("drawdown_limit_exceeded")
        if exposure["total_exposure_ratio"] >= 0.30:
        ...
        elif selection.selected_strategy_id.startswith("defensive") or analysis.confidence < 0.40:
            decision = AuditDecisionType.OBSERVE_ONLY
            approved = False
            risk_level = RiskLevel.LOW
            warnings.append("low_confidence_or_defensive_strategy")
        elif analysis.confidence < 0.60 or analysis.volatility_level == "high":
        ...
        approved_order_plan = {
            ...
            "max_position_ratio": adjustments[0].adjusted if adjustments else 0.10,
            "leverage": 1,
            "time_in_force": "GTC",
            "allow_execution": approved,
        }
        checks = AuditChecks(
            global_risk_limit_ok=global_risk["daily_drawdown_ratio"] < 0.08 and not self.kill_switch_service.is_enabled(),
            strategy_applicability_ok=all(applicability.values()),
            exposure_ok=exposure["total_exposure_ratio"] < 0.30,
            liquidity_ok=analysis.liquidity_level != "low",
            event_risk_ok="elevated_volatility" not in analysis.risk_flags,
        )
