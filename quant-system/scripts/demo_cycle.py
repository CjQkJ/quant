...
def print_text_summary(
    ...
    print(f"- fallback: {result.selection.fallback_candidate.strategy_name if result.selection.fallback_candidate else '无'}")
    print(f"- challenger: {result.selection.challenger_candidate.strategy_name if result.selection.challenger_candidate else '无'}")
    print(f"- no-trade candidate: {result.selection.no_trade_candidate.strategy_name if result.selection.no_trade_candidate else '无'}")
    ...
    print(f"- 下一步动作: {result.audit.next_action}")
    print(f"- 补充上下文要求: {', '.join(result.audit.context_requirements) if result.audit.context_requirements else '无'}")
    ...
    print(f"- 累计手续费: {result.account_snapshot.fee_paid_total:.2f}")
    print(f"- 系统状态: {result.monitor.system_status}")
    print(f"- Kill Switch: {result.monitor.kill_switch}")
    stale_sources = [item.source for item in result.monitor.source_freshness if item.is_stale]
    print(f"- 数据源 freshness: {'正常' if not stale_sources else ', '.join(stale_sources)}")
    print(f"- 监控建议: {', '.join(item.action for item in result.monitor.suggestions) if result.monitor.suggestions else '无'}")
    print("")
    print("提示")
