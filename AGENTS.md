...
6. **立即部署适配到 Codex 中投入使用**：
   - Skill → 在 `.Codex/skills/<skill-name>/SKILL.md` 创建配置文件
   - MCP → 在项目根目录 `.mcp.json` 或通过 `Codex mcp add` 命令注册

开发完成 ≠ 交付完成。必须经过审查、烟测、全流程跑通，并配置到 Codex 后才算真正完成。

## 开发规范

### 中文要求（强制）

- **所有文档**使用中文（README、AGENTS.md、docs/、SKILL.md 等）
...
- 每个 Skill / MCP 开发完成并通过烟测后，**必须立即部署适配到 Codex 项目中投入使用**
- Skill 配置到 `.Codex/skills/<skill-name>/SKILL.md`
- MCP 配置到 `.mcp.json` 或通过 `Codex mcp add` 注册
